...
import {Alert, Keyboard, RecyclerViewBackedScrollView} from 'react-native';
...
class Container extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      title: mode === 'create' ? review.title : '',
  ...
  _submit = async () => {
    ...
    if (!isSubmittingReview) {
      if (rating > 0) {
        if (rating || expression || content || title) {
          this.setState({
            isSubmittingReview: true,
          });
          const result = await createArtworkReview(
            id,
            rating,
            expression,
            content,
    ...
    if (!isSubmittingReview) {
      if (rating > 0) {
        if (rating || expression || content || title) {
          ...
          const result = await updateArtworkReview(
            artwork.id,
            id,
            rating,
            expression,
    ...
    if (!isSubmittingReply) {
      if (selectedReply.id) {
            ...
            this.state.replies.map(rep => {
              if (rep.id === selectedReply.id) {
                newReplist.push({
                  ...
                  initial_replies: [...rep.initial_replies, result.reply],
                });
              } else {
                newReplist.push(rep);
              }
        ...
        if (id && contentReply) {
          ...
          if (result.status === 'ok') {
            this.setState({
              replies: [...this.state.replies, result.reply],
              isSubmittingReply: false,
              contentReply: '',
              selectedReply: {},
            });
