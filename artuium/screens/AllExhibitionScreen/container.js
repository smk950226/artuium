...
class Container extends Component{
    ...
    componentDidMount = async() => {
        ...
        const exhibitions = await getExhibitionList('all', filter, period.value, scale.value, region.value)
        this.setState({
            exhibitions,
            loading: false
        })
    ...
    componentDidUpdate = async(prevProps, prevState) => {
        if((prevState.filter !== this.state.filter) || (prevState.period !== this.state.period) || (prevState.scale !== this.state.scale) || (prevState.region !== this.state.region)){
            this.setState({
                loading: true,
                page: 1,
                hasNextPage: true,
    ...
    _exhibitionMore = async() => {
        ...
        const { page, hasNextPage, isLoadingMore, filter, period, scale, region } = this.state;
        if(hasNextPage){
            if(!isLoadingMore){
                await this.setState({
                    isLoadingMore: true
                });
                const result = await getExhibitionListMore('all', filter, period.value, scale.value, region.value, page+1);
                if(result){
                    await this.setState({
