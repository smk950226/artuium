...
import AllExhibitioinReviewScreen from './presenter';
...
class Container extends Component{
    ...
    componentDidMount = async() => {
        ...
        const reviews = await getReviewList('exhibition', filter)
        this.setState({
            reviews,
            loading: false
        })
    ...
    _reviewMore = async() => {
        ...
        if(hasNextPage){
            if(!isLoadingMore){
                ...
                const result = await getReviewListMore('exhibition', filter, page+1);
                if(result){
                    await this.setState({
